...
contract Pair is ReentrancyGuard {
    ...
    function _getAgentAmountOut(uint256 assetAmountIn) private view returns (uint256) {
        ...
        // Calculate effective asset reserves
        uint256 effectiveAssetReserves = reserveAsset + initialReserveAsset;
        
        // Calculate numerator: input amount * current agent token reserves * BASIS_POINTS
        uint256 numerator = assetAmountIn * reserveAgent * BASIS_POINTS;
        
        // Compute denominator: (amplified reserves) + (input amount * BASIS_POINTS)
        uint256 denominator = (effectiveAssetReserves * impactMultiplier) + 
                            (assetAmountIn * BASIS_POINTS);
    ...
    function _getAssetAmountOut(uint256 agentAmountIn) private view returns (uint256) {
        ...
        require(agentAmountIn < reserveAgent, "EXCESSIVE_INPUT_AMOUNT");

        // Calculate effective asset reserves
        uint256 effectiveAssetReserves = reserveAsset + initialReserveAsset;

        // Compute numerator: input * reserves * impact multiplier
        uint256 numerator = agentAmountIn * effectiveAssetReserves * impactMultiplier;

        // Compute denominator: (reserve - input) * BASIS_POINTS
        uint256 denominator = (reserveAgent - agentAmountIn) * BASIS_POINTS;
